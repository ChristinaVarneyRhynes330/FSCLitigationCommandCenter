...
export default function App() {
  ...
  const handleAddEvidence = (evidence: Omit<Evidence, "batesNumber">) => {
    const batesNumber = `${state.batesPrefix}-${String(
      state.batesCounter,
    ).padStart(3, "0")}`;
    setState((prev) => ({
      ...prev,
  ...
  const handleEmergencyGuidance = async (scenario: string): Promise<string> => {
    const prompts: Record<string, string> = {
      missed_deadline:
        "An attorney just realized they missed a critical filing deadline. Provide immediate emergency triage: 1) Immediate actions (next 24 hours), 2) Risk assessment, 3) Damage control strategies (motion for extension, excusable neglect under FRCP 6(b)), 4) Long-term remediation.",
      motion_dismiss:
        "Opposing counsel just filed a motion to dismiss. Provide: 1) Immediate response timeline (typically 21 days under FRCP 12), 2) Key arguments to address, 3) Evidence to marshal, 4) Strategic considerations (amend complaint vs. oppose motion).",
      conferral_fail:
        "Good faith conferral under FRCP 26(f) or local meet-and-confer rules failed. Provide: 1) Documentation requirements, 2) How to demonstrate good faith effort, 3) Next steps (file motion with certificate of conferral), 4) Protecting the record.",
    };

    const prompt =
      prompts[scenario] || "Provide emergency litigation guidance.";
